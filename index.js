const seriesSkill = [
    ...
    {
        name:'煌雷竜の力',
        keywords:['こうらいりゅうのちから','らいらいきょうめい']
    },
    {
        name:'雷顎竜の闘志',
...
const loadData = () => {
    ...
    const moveCount = Math.min(...inputData.map(data => {
        const idx = data.skillData.findIndex(skill => !skill.checked);
        return idx === -1 ? data.skillData.length : idx;
    }));
